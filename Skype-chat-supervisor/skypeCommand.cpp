...
QString skypeCommand::GET_CONTACT_STATUS(QString contactName) { 
  return "GET USER "+contactName+" ONLINESTATUS";
}
QString skypeCommand::GET_CHATMESSAGE_PROPERTY(QString ChatMessageId, QString property){
    return "GET CHATMESSAGE "+ChatMessageId+" "+property;
}
QString skypeCommand::SEND_CHATMESSAGE(QString ChatMessageName, QString message){
    return "CHATMESSAGE "+ChatMessageName+" "+message;
}

QString skypeCommand::CALL_CONTACT(QString contactName) {
...
bool skypeResponse::parse(const QString &message) {
  QRegExp exp;
  QStringList list;
  QString CommandName;
  QString msg = message;
  ...
  if ( msg.indexOf("USERSTATUS") == 0 ) { 
    msg.remove( QRegExp("^USERSTATUS ") );
    if ( msg == "ONLINE" ) {
      cStatus = online;
      /*qDebug() << "skypeResponse::parse(): userstatus online";*/
    } else if ( msg == "OFFLINE" || msg == "SKYPEOUT" ) cStatus = offline;
    else if ( msg == "AWAY" ) cStatus = away;
    else cStatus = other;
    Type = SK_USER_STATUS;
    return true;
  }
  ...
  if ( msg.indexOf("USER ") == 0 ) { 
    ...
    if ( exp.exactMatch(msg) ) { 
      list = exp.capturedTexts();
      ContactName = list[1];
      if ( list[2] == "ONLINESTATUS" ) { 
	if ( list[3] == "ONLINE" ) cStatus = online;
        else if ( list[3] == "OFFLINE" || list[3] == "SKYPEOUT" ) cStatus = offline;
        else cStatus = other;
	Type = SK_CONTACT_STATUS;
      } else { 
	Type = SK_UNKNOWN;
      }
    } else { 
  ...
  if ( msg.indexOf("CURRENTUSERHANDLE") == 0 ) { 
    msg.remove( QRegExp("^CURRENTUSERHANDLE ") );
    ContactName = msg;
    Type = SK_CURRENT_USER;
    return true;
  }
  ...
  if ( msg.indexOf("CHATMESSAGE ") == 0 ) {
    ...
    if ( exp.exactMatch(msg) ) {
      ...
      ChatMessageProperty = list[2];
//      if (list[2] = "BODY"){
//          описать все предопределенные как по статусу:
//      }
      if (list[2] == "STATUS"){
          if      (list[3] == "RECEIVED"){mStatus = SK_CMS_RECEIVED;}
          else if (list[3] == "SENDING") {mStatus = SK_CMS_SENDING;}
          else if (list[3] == "SENT")    {mStatus = SK_CMS_SENT;}
          else if (list[3] == "READ")    {mStatus = SK_CMS_READ;}
      }else{
          Msg = list[3];
      }
