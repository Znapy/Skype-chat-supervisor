...
skypeComm::skypeComm() { 
  ...
  connect( msg, SIGNAL( gotMessage(WId, const QString &) ), this, SLOT( processX11Message(WId, const QString &) ) );
  skype_win = 0;
}
...
bool skypeComm::attachToSkype( int ) {
  ...
  {
    skype_win = (WId) -1;
    qDebug("skypeComm::connectToInstance(): Skype not detected, status %d\n", status );
    return false;
  } else  {
    skype_win = * (const unsigned long *) prop & 0xffffffff;
    qDebug("skypeComm::connectToInstance(): Skype instance found, window id %d\n", (int) skype_win);
    emit attachedToSkype();
    return true;
  }
}


void skypeComm::processX11Message(WId win, const QString &message) {
